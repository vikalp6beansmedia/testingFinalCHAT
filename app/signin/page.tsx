...
export default function SignInPage() {
  ...
  useEffect(() => {
    ...
    smartRedirect(role, tier);
  }, [session, status]);

  function smartRedirect(role: string, tier: string) {
    if (role === "ADMIN") { router.replace("/admin/posts"); return; }
    if (role === "CREATOR") { router.replace("/admin/posts"); return; }
    // Fan — go to creator page
  ...
  async function onGoogle() {
    setMsg(null);
    // After Google auth, NextAuth will redirect — we handle via callback
